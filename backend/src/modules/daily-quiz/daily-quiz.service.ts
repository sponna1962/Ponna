...
export class DailyQuizService {
  parseAndValidateCsv(csvText: string): { rows: ParsedRow[]; errors: string[] } {
    ...
    if (records.length !== 10) errors.push(`Expected exactly 10 questions, found ${records.length}.`);
    const rows: ParsedRow[] = [];
    records.forEach((r, i) => {
      const rowNum = i + 1;
      const required = ['Tamil Question','TA Option A','TA Option B','TA Option C','TA Option D','English Question','EN Option A','EN Option B','EN Option C','EN Option D','Correct Option','TA Explanation','EN Explanation'];
      for (const col of required) if (!r[col]?.trim()) errors.push(`Row ${rowNum}: "${col}" is empty — every field is required, including both explanations.`);
      const correct = r['Correct Option']?.trim().toUpperCase();
      if (correct && !['A','B','C','D'].includes(correct)) errors.push(`Row ${rowNum}: "Correct Option" must be A, B, C, or D — got "${r['Correct Option']}".`);
      rows.push({
        questionTextTa: r['Tamil Question']?.trim() ?? '', optionATa: r['TA Option A']?.trim() ?? '', optionBTa: r['TA Option B']?.trim() ?? '', optionCTa: r['TA Option C']?.trim() ?? '', optionDTa: r['TA Option D']?.trim() ?? '',
        questionTextEn: r['English Question']?.trim() ?? '', optionAEn: r['EN Option A']?.trim() ?? '', optionBEn: r['EN Option B']?.trim() ?? '', optionCEn: r['EN Option C']?.trim() ?? '', optionDEn: r['EN Option D']?.trim() ?? '',
        correctOption: (correct as CorrectOption) ?? 'A', explanationTa: r['TA Explanation']?.trim() ?? '', explanationEn: r['EN Explanation']?.trim() ?? '',
      });
    });
    return { rows, errors };
  }
  ...
  async createDailyQuiz(quizDateStr: string, publishTimeIst: string, rows: ParsedRow[], quizType: DailyQuizType = DailyQuizType.DAILY_QUIZ) {
    ...
    return prisma.dailyQuiz.create({ data: { quizDate: new Date(quizDateStr), quizType, publishAt, expiresAt, status: DailyQuizStatus.SCHEDULED, questions: { create: rows.map((row, i) => ({ sequenceNumber: i + 1, ...row })) } }, include: { questions: true } });
  }

  private rowsToCsvForRevalidation(rows: ParsedRow[]): string {
    const header = 'Tamil Question,TA Option A,TA Option B,TA Option C,TA Option D,English Question,EN Option A,EN Option B,EN Option C,EN Option D,Correct Option,TA Explanation,EN Explanation';
    const esc = (v: string) => `"${v.replace(/"/g, '""')}"`;
    return [header, ...rows.map((r) => [r.questionTextTa,r.optionATa,r.optionBTa,r.optionCTa,r.optionDTa,r.questionTextEn,r.optionAEn,r.optionBEn,r.optionCEn,r.optionDEn,r.correctOption,r.explanationTa,r.explanationEn].map(esc).join(','))].join('\n');
  }

  async listDailyQuizzes(quizType?: DailyQuizType) {
    return prisma.dailyQuiz.findMany({ where: quizType ? { quizType } : undefined, orderBy: { quizDate: 'desc' }, include: { questions: false, _count: { select: { attempts: true } } } });
  }
  ...
  async publishNow(id: string) {
    const quiz = await prisma.dailyQuiz.findUniqueOrThrow({ where: { id }, include: { questions: true } });
    if (quiz.questions.length !== 10) throw new DailyQuizError(`Cannot publish — expected 10 questions, found ${quiz.questions.length}.`);
    if (quiz.status === DailyQuizStatus.EXPIRED) throw new DailyQuizError('Cannot publish an expired quiz.');
    const publishAt = new Date();
    const expiresAt = new Date(publishAt.getTime() + 24 * 60 * 60 * 1000);
    return prisma.dailyQuiz.update({ where: { id }, data: { status: DailyQuizStatus.PUBLISHED, publishAt, expiresAt } });
  }
  ...
  async runStatusSweep() {
    const now = new Date();
    const publishedCount = await prisma.dailyQuiz.updateMany({ where: { status: DailyQuizStatus.SCHEDULED, publishAt: { lte: now } }, data: { status: DailyQuizStatus.PUBLISHED } });
    const expiredCount = await prisma.dailyQuiz.updateMany({ where: { status: DailyQuizStatus.PUBLISHED, expiresAt: { lte: now } }, data: { status: DailyQuizStatus.EXPIRED } });
  ...
  private async hasPaidAccess(userId: string): Promise<boolean> {
    ...
    const activeSub = await prisma.subscription.findFirst({ where: { userId, status: SubscriptionStatus.ACTIVE, cycleEnd: { gt: new Date() }, plan: { isFree: false } } });
    return !!activeSub;
  }
