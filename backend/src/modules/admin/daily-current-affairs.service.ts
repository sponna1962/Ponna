...
// It creates a scheduled DailyQuiz directly; it never creates Question rows
// and never sends anything to the normal Question/DRAFT bank.
...
Only use facts supported by the real search results. Do not invent events. Return fewer only if there are genuinely fewer than ${QUESTIONS_PER_DAY} suitable events.`;
  ...
  /** Generate today's Daily Quiz directly. Never creates normal Question rows. */
  async generateDailyQuiz(): Promise<{ created: number; quizId: string | null; quizDate: string; skippedNoResults: boolean }> {
    ...
    const quiz = await prisma.dailyQuiz.create({
      data: {
        ...
        // This is a DailyQuiz workflow state, not a Question-bank DRAFT.
        // Admin reviews and publishes/schedules from Daily Quiz itself.
