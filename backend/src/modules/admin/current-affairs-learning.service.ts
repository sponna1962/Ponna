...
type Verified = Candidate & { sourcePublishedAt: string; sourceUrl: string; verificationNote: string };
...
function window() { const end = new Date(); const start = new Date(end.getTime() - WINDOW_HOURS * 60 * 60_000); return { start: start.toISOString(), end: end.toISOString() }; }
...
export class CurrentAffairsLearningService {
  private async gemini(prompt: string, search: boolean, maxOutputTokens = 9000) {
    ...
    const body: any = { contents: [{ parts: [{ text: prompt }] }], generationConfig: { temperature: 0.1, maxOutputTokens } };
  ...
  async create(data: { date: string; category: string; headline: string; summary: string; sourceUrl?: string; examRelevanceNote?: string; verifiedAt?: string }) {
    ...
    const existing = await prisma.currentAffairsItem.findMany({ where: { date: istToUtc(data.date) }, select: { headline: true } });
    const fp = fingerprint(data.date, headline);
    if (existing.some((row) => fingerprint(data.date, row.headline.replace(/^\[[^\]]+\]\s*/, '')) === fp)) throw new Error('இந்த நிகழ்வு ஏற்கனவே சேர்க்கப்பட்டுள்ளது.');
    return prisma.currentAffairsItem.create({ data: { date: istToUtc(data.date), headline: `[${category}] ${headline}`, summary: data.summary.trim(), sourceUrl: data.sourceUrl?.trim() || null, examRelevanceNote: data.examRelevanceNote?.trim() || null, verifiedAt: new Date(data.verifiedAt || new Date().toISOString()) } });
  }

  async delete(id: string) { await prisma.currentAffairsItem.delete({ where: { id } }); }

  private async discoverAndVerify(previousHeadlines: string[]) {
    const { start, end } = window();
    const previousText = previousHeadlines.slice(0, 300).join('\n');
    ...
    return (verification.verifiedEvents ?? []).filter((item) => {
      ...
      const key = fingerprint(item.eventDateTime, item.headline);
  ...
  async generateDaily() {
    ...
    const existingHeadlines = existing.map((row) => row.headline.replace(/^\[[^\]]+\]\s*/, ''));
    const existingFingerprints = new Set(existing.map((row) => fingerprint(row.date.toISOString(), row.headline.replace(/^\[[^\]]+\]\s*/, ''))));
    const verified = await this.discoverAndVerify(existingHeadlines);
    const fresh = verified.filter((item) => !existingFingerprints.has(fingerprint(item.eventDateTime, item.headline)));

    if (fresh.length < MIN_ITEMS) return { created: 0, skipped: true, verified: fresh.length, reason: `Only ${fresh.length} genuinely new verified events were available; nothing was created.` };
    ...
    for (const item of selected) {
      ...
      try {
        await this.create({ date: item.eventDateTime.slice(0, 10), category: item.category, headline: item.headline, summary: facts, sourceUrl: item.sourceUrl, examRelevanceNote: relevance, verifiedAt: new Date().toISOString() });
        created += 1;
      } catch (err: any) {
        if (!String(err?.message).includes('ஏற்கனவே')) throw err;
      }
    }
    return { created, skipped: false, verified: fresh.length, date: new Date().toISOString() };
  }
}
