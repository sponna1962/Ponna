...
export class CurrentAffairsLearningService {
  ...
  async create(data: { date: string; category: string; headline: string; summary: string; headlineEn?: string; summaryEn?: string; sourceUrl?: string; examRelevanceNote?: string; examRelevanceNoteEn?: string; verifiedAt?: string }) {
    ...
    return prisma.currentAffairsItem.create({
      data: {
        ...
        headlineEn: data.headlineEn?.trim() ? `[${category}] ${data.headlineEn.trim()}` : null,
        summaryEn: data.summaryEn?.trim() || null,
        sourceUrl: data.sourceUrl?.trim() || null,
        examRelevanceNote: data.examRelevanceNote?.trim() || null,
        examRelevanceNoteEn: data.examRelevanceNoteEn?.trim() || null,
  ...
  async generateDaily() {
    ...
    for (const item of selected) {
      ...
      const factsEn = (item.factsEn ?? []).slice(0, 3).join(' ');
      ...
      const relevanceEn = factsEn
        ? `Important for the exam: ${factsEn}\nRemember: ${item.category} — ${item.location || 'India'} — ${item.eventDateTime.slice(0, 10)}`
        : undefined;
      try {
        await this.create({
          ...
          summaryEn: factsEn || undefined,
          sourceUrl: item.sourceUrl,
          examRelevanceNote: relevance,
          examRelevanceNoteEn: relevanceEn,
