...
export default function CurrentAffairsPage() {
  ...
  return (
    <main style={{ minHeight: '100vh', background: COLORS.paper, color: COLORS.ink, fontFamily: FONT_FAMILY }}>
      <BitterFontLinks />
      <header style={{ position: 'sticky', top: 0, zIndex: 20, background: 'rgba(255,253,247,0.96)', borderBottom: `1px solid ${COLORS.border}`, backdropFilter: 'blur(8px)' }}>
        <div style={{ maxWidth: 900, margin: '0 auto', padding: '13px 18px', display: 'flex', alignItems: 'center', gap: 14 }}>
          <StudentMenu />
          <div>
            <div style={{ fontSize: 21, fontWeight: 800 }}>நடப்பு நிகழ்வுகள்</div>
            <div style={{ fontSize: 12, color: COLORS.inkMuted }}>தினமும் படித்து தெரிந்துகொள்ளுங்கள்</div>
          </div>
        </div>
      </header>

      <section style={{ maxWidth: 900, margin: '0 auto', padding: '22px 18px 70px' }}>
        <div style={{ background: '#f6efe1', border: `1px solid ${COLORS.border}`, borderRadius: 14, padding: '15px 16px', marginBottom: 22 }}>
          <strong>📖 இது படிப்பதற்கான பகுதி.</strong>
          <div style={{ marginTop: 5, fontSize: 13.5, lineHeight: 1.65, color: COLORS.inkMuted }}>
            தேர்வுக்கு பயன்படும் முக்கியமான நிகழ்வுகள் மட்டும் சுருக்கமாக வழங்கப்படுகின்றன. புதிய நிகழ்வுகள் மேலே சேரும்; பழைய நிகழ்வுகள் கீழே தொடர்ந்து இருக்கும்.
          </div>
        </div>
        ...
        {grouped.map((group) => (
          <section key={group.date} style={{ marginBottom: 32 }}>
            ...
            {group.items.map((item, index) => {
              ...
              return (
                <article key={item.id} style={{ background: '#fff', border: `1px solid ${COLORS.border}`, borderRadius: 14, padding: '17px 17px 15px', marginBottom: 12, boxShadow: '0 2px 8px rgba(0,0,0,0.035)' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 7 }}>
                    <span style={{ fontSize: 11, fontWeight: 800, color: COLORS.gold, background: '#fbf3df', borderRadius: 999, padding: '4px 9px' }}>{category}</span>
                    <span style={{ fontSize: 11, color: COLORS.inkMuted }}>#{index + 1}</span>
                  </div>
                  ...
                  {relevance && (
                    <div style={{ marginTop: 12, padding: '9px 11px', background: '#f7f9fc', borderRadius: 9, fontSize: 13, lineHeight: 1.55 }}>
                      <strong>தேர்வுக்கு முக்கியம்:</strong> {relevance}
                    </div>
                  )}
                  {memory && (
                    <div style={{ marginTop: 7, fontSize: 12.5, color: COLORS.inkMuted }}>
                      <strong>நினைவில் வைக்க:</strong> {memory}
                    </div>
                  )}
                  {item.sourceUrl && (
                    <a href={item.sourceUrl} target="_blank" rel="noreferrer" style={{ display: 'inline-block', marginTop: 10, fontSize: 12, color: COLORS.gold, textDecoration: 'none', fontWeight: 700 }}>
                      ஆதாரம் ↗
                    </a>
                  )}
                </article>
